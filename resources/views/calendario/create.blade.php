...
@section('title', 'Agendar Nueva Cita - CRM')
...
<style>
    ...
    /* Estilos TomSelect */
    .ts-wrapper .ts-control { background-color: var(--cf-bg) !important; border: 1px solid #475569 !important; color: var(--cf-text) !important; border-radius: 9px !important; min-height: 50px !important; padding: 10px 14px !important; font-size: 14px !important; }
...
<section class="comi-cita-form" aria-labelledby="cita-create-title">
    ...
    <header class="cf-header">
        ...
        <p class="cf-subtitle">Programa una consulta o reunión con un prospecto o solicitante de la empresa.</p>
    </header>
    ...
    @if ($errors->any())
        <div class="cf-alert" role="alert">
            No se pudo guardar la cita. Revisa los datos ingresados.
            <ul>
    ...
    <div class="cf-card">
        <div class="cf-card-heading">
            ...
            <div>
                <h2 class="cf-card-title">Detalles de la Cita / Consulta</h2>
                <p class="cf-card-note">Ingresa la información de contacto del solicitante y los detalles de la cita.</p>
            </div>
        </div>
        ...
        <form action="{{ route('calendario.store') }}" method="POST" class="cf-form">
            ...
            <div class="cf-body">
                ...
                <!-- Sección 1: Información del Solicitante / Prospecto -->
                <fieldset class="cf-section">
                    <legend class="cf-legend"><span class="cf-step" aria-hidden="true">1</span> Datos del solicitante</legend>
                    
                    @if(isset($prospectos) && count($prospectos) > 0)
                        <div style="margin-bottom: 20px;">
                            <label for="id_prospecto" class="cf-label">Seleccionar prospecto/contacto registrado</label>
                            <select name="id_prospecto" id="id_prospecto">
                                <option value="" disabled selected>Buscar o seleccionar prospecto...</option>
                                @foreach($prospectos as $prospecto)
                                    <option value="{{ $prospecto->id }}" {{ old('id_prospecto') == $prospecto->id ? 'selected' : '' }}>
                                        {{ $prospecto->nombre }} ({{ $prospecto->telefono ?? $prospecto->correo ?? 'Sin contacto' }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="cf-grid">
                        <div>
                            ...
                            <div class="cf-field cf-field-single">
                                ...
                                <input type="text" name="nombre_solicitante" id="nombre_solicitante" class="cf-input" value="{{ old('nombre_solicitante') }}" placeholder="Ej. Juan Pérez" required>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="cf-field cf-field-single">
                                ...
                                <input type="tel" name="telefono_solicitante" id="telefono_solicitante" class="cf-input" value="{{ old('telefono_solicitante') }}" placeholder="Ej. 5512345678">
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="margin-top: 20px;">
                        ...
                        <div class="cf-field cf-field-single">
                            ...
                            <input type="email" name="correo_solicitante" id="correo_solicitante" class="cf-input" value="{{ old('correo_solicitante') }}" placeholder="ejemplo@correo.com">
                        </div>
                    </div>
                </fieldset>

                <!-- Sección 2: Programación y Asunto -->
                <fieldset class="cf-section">
                    <legend class="cf-legend"><span class="cf-step" aria-hidden="true">2</span> Programación de la consulta</legend>

                    <div style="margin-bottom: 20px;">
                        <label for="motivo" class="cf-label">Motivo / Consulta <span class="cf-required" aria-hidden="true">*</span></label>
                        <div class="cf-field">
                            <i class="bi bi-chat-left-text" aria-hidden="true"></i>
                            <textarea name="motivo" id="motivo" class="cf-input" rows="2" placeholder="Ej. Consulta sobre servicios de soporte, propuesta comercial..." required>{{ old('motivo') }}</textarea>
                        </div>
                    </div>
                ...
                <fieldset class="cf-section">
                    ...
                    <div>
                        <label for="observaciones" class="cf-label">Observaciones o requerimientos</label>
                        <div class="cf-field">
                            <i class="bi bi-card-text" aria-hidden="true"></i>
                            <textarea name="observaciones" id="observaciones" class="cf-input" rows="3" placeholder="Detalles de la consulta o preparación previa...">{{ old('observaciones') }}</textarea>
                        </div>
                    </div>
                </fieldset>
            ...
            <div class="cf-footer">
                ...
                <div class="cf-actions">
                    ...
                    <button type="submit" class="cf-btn cf-btn-primary">
                        <i class="bi bi-check2" aria-hidden="true"></i> Agendar consulta
                    </button>
                </div>
            </div>
        </form>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const prospectoSelect = document.getElementById('id_prospecto');
        if (prospectoSelect) {
            new TomSelect('#id_prospecto', {
                create: false,
